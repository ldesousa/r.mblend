...
def main():
    ...
    if(far_edge < 0 or far_edge > 100):
        gscript.error(_("[r.mblend] ERROR: far_edge must be a percentage between 0 and 100."))
        exit()
        
    if(inter_points < 0):
        gscript.error(_("[r.mblend] ERROR: inter_points must be a positive integer."))
        exit()
    ...
    gscript.message(_("[r.mblend] Resampling low resolution raster to higher resolution"))
    ...
    gscript.message(_("[r.mblend] Computing inner buffer to interpolation area"))
    ...
    gscript.message(_("[r.mblend] Selecting far edge points (using cut-off percentage)"))
    ...
    gscript.message(_("[r.mblend] Interpolating smoothing surface. This might take a while..."))
